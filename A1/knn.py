...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      for j in range(num_test):
        dists[i,j]=torch.sqrt(torch.sum((x_train_flatten[i]-x_test_flatten[j])**2))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_flatten=x_test.reshape(num_test,-1)
    for j in range(num_test):
      dists[:,j]=(x_train_flatten-x_test_flatten[j]).pow(2).sum(dim=1).pow(1/2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flatten=x_train.reshape(num_train,-1)
    x_test_flatten=x_test.reshape(num_test,-1)
    dists=(x_train_flatten.pow(2).sum(dim=1).reshape(-1,1)-2*x_train_flatten.mm(x_test_flatten.T)+x_test_flatten.pow(2).sum(dim=1).reshape(1,-1)).pow(1/2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    val,idx=dists.topk(k,dim=0,largest=False)
    for i in range(idx.shape[1]):
      _,label=torch.max(y_train[idx][:,i].bincount(),dim=0)
      y_pred[i]=label
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        x_valid,y_valid=x_train_folds[i],y_train_folds[i]
        train_x_without_i=[x for x in x_train_folds]
        train_x_without_i.pop(i)
        train_x=torch.cat(train_x_without_i,dim=0)
        train_y_without_i=[y for y in y_train_folds]
        train_y_without_i.pop(i)
        train_y=torch.cat(train_y_without_i,dim=0)
        
        classifier = KnnClassifier(train_x, train_y)
        list_i.append(classifier.check_accuracy(x_valid, y_valid, k))
      k_to_accuracies[k]=list_i
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
