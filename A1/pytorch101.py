...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    if stop-start<10:
      ...
      x=torch.arange(10,stop,10,dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    mu_j=x.sum(dim=0)/M
    # print(mu_j)
    sigma_j=torch.sqrt(torch.sum((x-mu_j)**2,dim=0)/(M-1))
    # print(x-mu_j,sigma_j)
    y=(x-mu_j)/sigma_j
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y=torch.tensor([x.sum()/i for x, i in List, ls])
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
