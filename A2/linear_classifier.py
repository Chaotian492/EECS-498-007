...
def svm_loss_naive(W: torch.Tensor, X: torch.Tensor, y: torch.Tensor, reg: float):
    ...
    loss /= num_train
    # like loss, we need divide by num_train
    dW /= num_train
...
def svm_loss_vectorized(W: torch.Tensor, X: torch.Tensor, y: torch.Tensor, reg: float):
    ...
    num_classes = W.shape[1]
    num_train = X.shape[0]
    idx0_num = torch.arange(num_train)
    scores = X.mm(W)
    correct_class_scores = scores[idx0_num, y]

    # margin = scores - correct_class_scores.reshape(-1,1) + 1
    # margin[margin<=0] = 0
    # 合并
    margin = torch.maximum(
        torch.zeros_like(scores), scores - correct_class_scores.reshape(-1, 1) + 1
    )

    margin[idx0_num, y] = 0

    loss = margin.sum() / num_train + reg * torch.sum(W * W)
    # print(num_classes, num_train, scores.shape, loss.shape)
    ...
    mask[idx0_num, y] = -count
    dW = X.T.mm(mask) / num_train + reg * 2 * W
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    y_pred_value = X.mm(W)
    y_pred = torch.argmax(y_pred_value, dim=1)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-3, 1e-2, 1e-3, 2e-3, 6e-3]
    regularization_strengths = [1e-1, 1e-3, 5e-3, 6e-4, 3e-5]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def softmax_loss_naive(W: torch.Tensor, X: torch.Tensor, y: torch.Tensor, reg: float):
    ...
    num_classes = W.shape[1]
    for i in range(num_train):
        sorce = W.t().mv(X[i])  # sorce -> (C,)
        sorce -= sorce.max()
        loss -= torch.log(torch.exp(sorce[y[i]]) / sorce.exp().sum())
        for j in range(num_classes):
            dW[:, j] += X[i] * torch.exp(sorce[j]) / torch.exp(sorce).sum()
        dW[:, y[i]] -= X[i]
    loss /= num_train
    loss += reg * torch.sum(W * W)
    dW /= num_train
...
def softmax_loss_vectorized(
    ...
    num_train = X.shape[0]
    idx0_num = torch.arange(num_train)
    score = X.mm(W)  # sorce -> (N,C)
    score -= torch.max(score, dim=1, keepdim=True).values

    loss = -(
        torch.log(score[idx0_num, y].exp() / score.exp().sum(1))
    ).sum() / num_train + reg * torch.sum(W * W)
    sorce_softmax = score.exp() / score.exp().sum(1, keepdim=True)
    sorce_softmax[idx0_num, y] -= 1
    dW = X.T.mm(sorce_softmax) / num_train + 2 * reg * W
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-3, 1e-6, 1e-8, 2e-4, 6e-3]
    regularization_strengths = [1e-1, 1e-2, 5e-3, 6e-4, 3e-5]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
