...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    FC_output = X.mm(W1) + b1  # -> (N,H)
    zeros = torch.zeros_like(FC_output)
    hidden = torch.maximum(FC_output, zeros)
...
def nn_forward_backward(
    ...
    softmax = scores[idx, y].exp() / scores.exp().sum(dim=1)  # sofrmax -> (N,)
    # L2 reg
    L2_W = torch.sum(W1 * W1) + torch.sum(W2 * W2)
    loss = -(softmax).log().sum() / N + reg * L2_W
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    scores_softmax = scores.exp() / scores.exp().sum(dim=1, keepdim=True)
    scores_softmax[idx, y] -= 1
    grad_scores = scores_softmax  # grad_scores -> (N,C)
    # grad W2 -> dL/dW2 = dL/ds * ds/dW2
    grads["W2"] = h1.T.mm(grad_scores) / N + 2 * reg * W2  # grad_W2 -> (H,C)

    # grad b2
    grads["b2"] = grad_scores.sum(dim=0) / N

    # grad W1 -> dL/dW1 = dL/dh * dh/dz * dz/dW1, dh/dz = ReLU'(z)
    grad_h1 = grad_scores.mm(W2.T)  # grad_h1 -> (N,H)
    grad_h1[h1 == 0] = 0  # grad ReLU -> (N,H)
    grads["W1"] = X.T.mm(grad_h1) / N + 2 * reg * W1  # grad_W1 -> (D,H)

    # grad b1
    grads["b1"] = grad_h1.sum(dim=0) / N
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_get_search_params():
    ...
    regularization_strengths = [1e-3, 1e-4, 1e-2]
    learning_rate_decays = [0.99, 0.95, 0.93, 0.90]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    ...
    for lr in lrs:
        for h in hs:
            for reg in regs:
                for lrd in lrds:
                    print(
                        "train with lr: {}, hs: {}, reg: {}, lrds: {}".format(
                            lr, h, reg, lrd
                        )
                    )
                    net = TwoLayerNet(
                    ...
                    y_test_pred = net.predict(data_dict["X_val"])
                    val_acc = (
                        100.0
                        * (data_dict["y_val"] == y_test_pred).double().mean().item()
                    )
