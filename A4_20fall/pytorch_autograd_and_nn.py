...
def initialize_three_layer_conv_part2(dtype=torch.float, device="cpu"):
    ...
    conv_w1 = nn.init.kaiming_normal_(
        torch.empty(channel_1, C, kernel_size_1, kernel_size_1, **dd)
    )
    conv_w1.requires_grad = True
    conv_b1 = nn.init.zeros_(torch.empty(channel_1, **dd))
    conv_b1.requires_grad = True
    conv_w2 = nn.init.kaiming_normal_(
        torch.empty(channel_2, channel_1, kernel_size_2, kernel_size_2, **dd)
    )
    conv_w2.requires_grad = True
    conv_b2 = nn.init.zeros_(torch.empty(channel_2, **dd))
    conv_b2.requires_grad = True
    fc_w = nn.init.kaiming_normal_(torch.empty(num_classes, channel_2 * H * W, **dd))
    fc_w.requires_grad = True
    fc_b = nn.init.zeros_(torch.empty(num_classes, **dd))
...
class ThreeLayerConvNet(nn.Module):
    def __init__(self, in_channel, channel_1, channel_2, num_classes):
        ...
        self.conv1 = nn.Conv2d(
            in_channels=in_channel, out_channels=channel_1, kernel_size=5, padding=2
        )
        self.conv2 = nn.Conv2d(
            in_channels=channel_1, out_channels=channel_2, kernel_size=3, padding=1
        )
        self.fc = nn.Linear(in_features=channel_2 * 32**2, out_features=num_classes)
            ...
            OrderedDict(
                [
                    ...
                    (
                        "conv1",
                        (
                            nn.Conv2d(Cin, Cout, kernel_size=3, padding=1, stride=2)
                            if downsample
                            else nn.Conv2d(
                                Cin, Cout, kernel_size=3, padding=1, stride=1
                            )
                        ),
                    ),
        ...
        self.block = nn.Sequential(
            OrderedDict(
                [
                    ...
                    ("batchnorm2", nn.BatchNorm2d(num_features=Cout // 4)),
                    ("relu2", nn.ReLU()),
                    (
                        "conv2",
                        nn.Conv2d(Cout // 4, Cout // 4, kernel_size=3, padding=1),
                    ),
                    ("batchnorm3", nn.BatchNorm2d(num_features=Cout // 4)),
                    ("relu3", nn.ReLU()),
                    (
                        "conv3",
